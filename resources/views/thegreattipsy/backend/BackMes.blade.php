...
@forelse ($mes as $row)
                                            <tr id="tr_{{ $row->id }}">
                                                ...
                                                <td>@forelse(App\model\tgt\coupon::where('b_id',$row->id)->get() as $coup){{ $coup->code }}@if($coup->order_id>0) <a href="javascript:;" class="canelCoupon" data-id="{{ $row->id }}" data-code="{{ $coup->code }}"><i class="fa fa-times"></i></a> @endif<br >@empty 無優惠券 @endforelse</td>
