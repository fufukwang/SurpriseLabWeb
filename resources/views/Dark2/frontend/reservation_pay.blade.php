...
<div style="font-weight:bold;font-size:24px;">第階段7/4-9/16座位已全數額滿</div>
將不定時釋出座位。可留意訂位系統<br><br>
若無法訂位但也想購票者，可至<a href="pre-sale.html" style="color:#a67c52;">第二階段預售</a>購票 (連結至 Pre Dale頁面)<br>
第二階段將開放9/22-11/4之座位
                </div>
                <p>
...
<span class="alert-wrap alert-show" id="agree-error" style="display: none;"><br /><i class="fa fa-exclamation-triangle" aria-hidden="true"></i>{{ trans('front.reservation.slug1agAlert') }}</span>
            ...
            <div style="max-width:300px;display: none;" class="main-container" id="second-section">
                <table>
                    ...
                    <tr class="date_tr">
                        <td class="select-wrap">
                            <select name="dayparts" id="dayparts"></select>
                        </td>
                    </tr>
                    <tr style="display: none;" id="daypartsAlert">
                        <td>
                            <span class="alert-wrap alert-show">
                                <i class="fa fa-exclamation-triangle" aria-hidden="true"></i>
                                您選擇的時段訂位已滿
                    ...
                    <tr style="display: none;" id="pro_idAlert">
                        <td>
                            <span class="alert-wrap alert-show">
                                <i class="fa fa-exclamation-triangle" aria-hidden="true"></i>
...
<div class="col-sm-3">
    <div class="btn btn-standard" id="CouponBtn" style="margin: 25px 0px;">確認</div>
        ...
        <div class="lightbox2pay">
            <div class="lightbox-container" style="max-width: 360px;">
                <div class="lightbox-header">
                    信用卡付款
                </div>
...
<div style="text-align:center;"><a href="https://www.tappaysdk.com/tch" target="_blank"><img src="https://docs.tappaysdk.com/images/logo.png" width="150"></a></div>
                </div>
...
.bootstrap-datetimepicker-widget table td.disabled:hover {color: #F5F5F5;}
#dayparts option:disabled,#pro_id option:disabled {-webkit-border-radius:25px; -moz-border-radius:25px; border-radius:25px;color:red;}
...
$('#form_date').datetimepicker({
    ...
    $.get('/dininginthedark2/GetAjaxData',{'act':'getByday','day':event.date.format('YYYY-MM-DD'),'pople':$('#Pople').val()},function(data){
        $('#dayparts option').remove();
        $('#dayparts').append('<option value="">請選擇時段</option>');
        for(i=0;i<data.length;i++){
            $('#dayparts').append('<option value="'+data[i].dayparts+'">'+data[i].dayparts+'</option>');
        }
        if($('#dayparts').html()==''){ $('#dayparts').append('<option value="0">此日期無時段可供選擇</option>'); }
        $('#dartpartsAlert').hide();
    },'json');
...
$(function(){
    ...
    $('#Pople').bind('change',function(){
        ...
        $.get('/dininginthedark2/GetAjaxData',{'act':'getBypople','pople':val},function(data){
            var nowdata = [];
            spday = [],oneday = [];
            for(i=0;i<data.length;i++){
                nowdata.push(data[i].day);
                /*
                var tmp = data[i].day;
