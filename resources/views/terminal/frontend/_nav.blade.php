    <!-- nav bar -->
    <nav id="js-nav-bar" class="nav-wrap js-hidden-target">
        <div class="pc-nav-wrap">
            <a href="/terminal" class="logo-wrap">
                <img src="/terminalAssets/images/general/brand.png" class="style-normal" alt="落日轉運站">
                <img src="/terminalAssets/images/general/brand_hover.png" class="style-hover" alt="落日轉運站">
            </a>
        </div>
        <div class="js-mobile-nav mobile-nav-wrap">
            <div class="mobile-nav-bar-wrap">
                <a href="/terminal" class="logo-wrap">
                    <img src="/terminalAssets/images/general/brand.png" alt="落日轉運站">
                </a>
                <div class="js-nav-bar-btn menu-btn">
                    <div class="menu-btn-text">
                        <span class="text-menu">MENU</span>
                        <span class="text-section active">TICKET</span>
                    </div>
                    <div class="menu-btn-icon"></div>
                </div>
            </div>
            <div class="mobile-nav-menu-wrap">
                <ul class="menu-wrap js-anchor-menu">
                    <!-- javascript:// -->
                    <li class="menu-item active" data-id="ticket"><a href="https://m.me/surpriselabtw?ref=20230202">“TICKET”</a></li>
                    <li class="menu-item" data-id="about"><a href="/terminal#about">ABOUT</a></li>
                    <li class="menu-item" data-id="projects"><a href="/terminal#projects">PROJECTS</a></li>
                    <li class="menu-item" data-id="eapps-instagram-feed-1"><a href="/terminal#eapps-instagram-feed-1">REVIEW</a></li>
                    <li class="menu-item" data-id="discount"><a href="/terminal#discount">DISCOUNT</a></li>
                    <!-- <li class="menu-item" data-id="tour"><a href="/terminal#tour">TOUR</a></li> -->
                    <li class="menu-item" data-id="rules"><a href="/terminal#rules">RULES</a></li>
                    <li class="menu-item" data-id="contact"><a href="/terminal#contact">CONTACT</a></li>
                    <li class="menu-item" data-id="introduce"><a href="/terminal#introduce">TEAM</a></li>
                </ul>
            </div>
        </div>
    </nav>

    <!-- nav menu - pc -->
    <nav class="pc-nav-menu-wrap js-hidden-target">
        <ul class="menu-wrap js-anchor-menu">
            <li class="menu-item active"><a href="https://m.me/surpriselabtw?ref=20230202">TICKET</a></li>
            <li class="menu-item" data-id="about"><a href="/terminal#about">ABOUT</a></li>
            <li class="menu-item" data-id="projects"><a href="/terminal#projects">PROJECTS</a></li>
            <li class="menu-item" data-id="eapps-instagram-feed-1"><a href="/terminal#eapps-instagram-feed-1">REVIEW</a></li>
            <li class="menu-item" data-id="discount"><a href="/terminal#discount">DISCOUNT</a></li>
            <!-- <li class="menu-item" data-id="tour"><a href="/terminal#tour">TOUR</a></li> -->
            <li class="menu-item" data-id="rules"><a href="/terminal#rules">RULES</a></li>
            <li class="menu-item" data-id="contact"><a href="/terminal#contact">CONTACT</a></li>
            <li class="menu-item" data-id="introduce"><a href="/terminal#introduce">TEAM</a></li>
        </ul>
    </nav>