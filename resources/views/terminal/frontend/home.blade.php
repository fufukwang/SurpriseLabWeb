<!DOCTYPE html>
<html lang="en" class="js">
<head>
    <!-- Metadata -->
    <meta charset="UTF-8">
    <meta name="keywords" content="驚喜製造, 無光晚餐, 一人餐桌, 台北喝酒, 活動, 沉浸式體驗, 週末要幹麻, 好玩, 喝酒">
    <meta name="description" content="在黑暗中，熟悉都變陌生，習慣都成嘗試。用僅剩的感官，盡情探索與感受，重新認識餐桌上的一切與對面的那個人。《無光晚餐》每日為你準時熄燈、端上黑暗。">
    <meta property="og:type" content="website"/>
    <meta property="og:title" content="無光晚餐 Dining In The Dark - 驚喜製造 Surprise Lab."/>
    <meta property="og:site_name" content="無光晚餐 Dining In The Dark - 驚喜製造 Surprise Lab."/>
    <meta property="og:description" content="在黑暗中，熟悉都變陌生，習慣都成嘗試。用僅剩的感官，盡情探索與感受，重新認識餐桌上的一切與對面的那個人。《無光晚餐》每日為你準時熄燈、端上黑暗。"/>
    <meta property="og:url" content="https://www.surpriselab.com.tw/dininginthedark3"/>
    <meta property="og:image" content="https://www.surpriselab.com.tw/dininginthedark3/img/general/opengraph.png?v=0.1"/>
    <meta property="og:image:type" content="image/png"/>
    <meta property="og:image:width" content="600"/>
    <meta property="og:image:height" content="315"/>
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="facebook-domain-verification" content="mmtxvobzbh451wck03upjlbgtj1i2n" />
    <title>無光晚餐 Dining In The Dark - 驚喜製造 Surprise Lab.</title>

    <!-- Style -->
    <link rel="icon" href="/terminalAssets/img/general/favicon.png" type="image/x-icon">
    <link rel="stylesheet" href="/terminalAssets/css/plugins/bootstrap-4.1.0/bootstrap.css"/>
    <link rel="stylesheet" href="/terminalAssets/css/plugins/aos/aos.min.css"/>

    <!-- Loading Animation CSS -->
    <link rel="stylesheet" type="text/css" href="/terminalAssets/css/plugins/imagesloaded/loading.css"/>

    <!-- Flashlight CSS -->
    <link rel="stylesheet" href="/terminalAssets/css/flashlight.css?v=0.1"/>

    <!-- Custom CSS -->
    <link rel="stylesheet" href="/terminalAssets/css/style.css?v=0.14"/>
    <link rel="stylesheet" href="/terminalAssets/css/board-modal.css?v=0.1">

    <!-- Google Tag Manager -->
<script>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':
new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],
j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src=
'https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);
})(window,document,'script','dataLayer','GTM-NKG6X5Z');</script>
<!-- End Google Tag Manager -->
</head>

<body class="loading modal-open">
<!-- Google Tag Manager (noscript) -->
<noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-NKG6X5Z"
height="0" width="0" style="display:none;visibility:hidden"></iframe></noscript>
<!-- End Google Tag Manager (noscript) -->

    <!-- Header LOGO -->
    <header class="logo scroll-background-color">
        <div class="logo-wrapper">
            <div class="logo-inner">
                <a class="smoothScroll" href="#Intro">
                    <img class="img-fluid" src="/terminalAssets/img/general/logo_h.svg" alt="無光晚餐 Dining In The Dark">
                </a>
            </div>
            <button class="hamburger">
                <span class="current-section">About</span>
                <span class="current-menu">Menu</span>
                <span class="menu">
                    <span class="icon-bar"></span>
                    <span class="icon-bar"></span>
                </span>
            </button>
        </div>
    </header>

    <!-- Anchor menu -->
    <nav class="anchor-wrapper">
        <ul>
            <li class="d-none active">
                <a class="smoothScroll intro" href="#Intro">Intro</a>
            </li>
            <li>
                <a class="smoothScroll about" href="#About">About</a>
            </li>
            <li>
                <a class="smoothScroll ticket" href="#Ticket">Ticket</a>
            </li>
            <li>
                <a class="smoothScroll rules" href="#Rules">Rules</a>
            </li>
            <li>
                <a class="smoothScroll contact" href="#Contact">Contact</a>
            </li>
        </ul>
    </nav>

    <main class="sections fixed-scroll-anchor index-main">

        <div class="lighting-wrap">
            <div class="lighting-inner">
                <div class="mask-box">

                    <!-- Section0 Intro -->
                    <section class="anchor-section" id="Intro">

                        <h1>無光晚餐 Dining In The Dark</h1>
                        
                        <!-- Intro -->
                        <div class="fullpage section sec-intro">
                            <div class="intro-wrap">

                                <!-- 底圖 -->
                                <div class="slogan-img show-1 shadow">
                                    <img src="/terminalAssets/img/home/about/intro-slogan-shadow.png" alt="無光晚餐 Dining In The Dark">
                                    <img src="/terminalAssets/img/home/about/intro-slogan-shadow.png" alt="無光晚餐 Dining In The Dark">
                                </div>

                                <!-- 文字 -->
                                <div class="slogan-img show-1">
                                    <img src="/terminalAssets/img/home/about/intro-slogan.png" alt="無光晚餐 Dining In The Dark">
                                    <img src="/terminalAssets/img/home/about/intro-slogan.png" alt="無光晚餐 Dining In The Dark">
                                </div>
                            </div>

                            <a class="smoothScroll btn-scroll-down" href="#Experience">
                                <img class="img-fluid" src="/terminalAssets/img/icon/icon-arrow.svg" alt="scroll-down">
                            </a>
                        </div>
                    </section>

                    <!-- Section1 About -->
                    <section class="anchor-section" id="About">

                        <h2>About</h2>
                        
                        <!-- Exp -->
                        <div class="section sec-experience" id="Experience">

                            <div class="section-header">
                                <div class="section-title">
                                    <h3>體驗內容</h3>
                                    <h4>Experience</h4>
                                </div>
                            </div>

                            <div class="about-box">
                                <div class="container">
                                    <div class="row">
                                        <div class="col-md-4 col-12">
                                            <div>
                                                <div>
                                                    <img src="/terminalAssets/img/home/about/about-1.png" alt="九十分鐘">
                                                    <img src="/terminalAssets/img/home/about/about-1-shadow.png" alt="九十分鐘">
                                                </div>
                                                <p class="color-lightgray">
                                                    九十分鐘
                                                </p>
                                            </div>
                                        </div>
                                        <div class="col-md-4 col-12">
                                            <div>
                                                <div>
                                                    <img src="/terminalAssets/img/home/about/about-2.png" alt="五道料理">
                                                    <img src="/terminalAssets/img/home/about/about-2-shadow.png" alt="五道料理">
                                                </div>
                                                <p class="color-lightgray">
                                                    五道料理<br/>
                                                    <span class="color-k0">(葷/蛋奶素)</span>
                                                </p>
                                            </div>
                                        </div>
                                        <div class="col-md-4 col-12">
                                            <div>
                                                <div>
                                                    <img src="/terminalAssets/img/home/about/about-3.png" alt="無限驚喜">
                                                    <img src="/terminalAssets/img/home/about/about-3-shadow.png" alt="無限驚喜">
                                                </div>
                                                <p class="color-lightgray">
                                                    無限驚喜
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </section>

                    <!-- Section2 Ticket -->
                    <section class="anchor-section" id="Ticket">

                        <h2>Ticket</h2>

                        <!-- 售票狀況 -->
                        <div class="section sec-ticket-status">
                            <div class="container">

                                <div class="section-header">
                                    <div class="section-title">
                                        <h3>售票狀況</h3>
                                        <h4>Ticket Status</h4>
                                    </div>
                                </div>

                                <div class="time-counter">
                                    <p>剩餘時間</p>
                                    <div class="d-flex time-rest d-flex align-items-center justify-content-center">
                                        <span class="days">00</span>天
                                        <span class="hours">00</span>時
                                        <span class="minutes">00</span>分
                                        <span class="seconds">00</span>秒
                                    </div>
                                </div>

                                <div class="sale-progress">
                                    <div class="progress">
                                        <div class="progress-bar" role="progressbar" style="width: 0%;" aria-valuemax="100"></div>
                                    </div>

                                    <div class="sale-progress-info">
                                        <div class="d-inline-block d-sm-flex normal-lineHeight">
                                            <p>
                                                已售出 <span class="highlight total-sale">1600</span> 張<span class="over-2000">，還剩 <span class="highlight total-rest">400</span> 張</span>
                                            </p>
                                            <p class="ml-auto text-right color-k0 progress-percent-area">
                                                已完成 <span class="highlight progress-percent">80</span> %
                                            </p>
                                        </div>
                                        <div class="info-text">
                                            <p>* 現正販售</p>
                                            <ul>
                                                <li>
                                                    <p>雙人套票可劃位區間為：2022/10/12 - 2022/12/11</p>
                                                </li>
                                                <li>
                                                    <p>雙菜單套票可劃位區間為：2022/10/12 - 2023/10/31</p>
                                                </li>
                                            </ul>
                                            <p>* 可劃位區間將依照實際狀況加開或縮短</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <!-- 選擇票券 -->
                        <div class="section sec-ticket-type" id="Ticket-type">
                            <div class="container">

                                <div class="section-header">
                                    <div class="section-title">
                                        <h3>選擇票券</h3>
                                        <h4>Ticket Type</h4>
                                    </div>
                                    <p class="section-desc">
                                        無光晚餐皆為雙數訂位
                                    </p>
                                </div>

                                <div class="ticket-type">
                                    
                                    <div class="type-item type-matinee">
                                        <a href="https://surpriselab.backme.tw/checkout/2534/19986?locale=zh-TW" class="ticket-frame" target="_blank">
                                            <img class="img-fluid d-none d-sm-block" src="/terminalAssets/img/tickets/ticket-face-1.png?v=0.1" alt="雙人套票">
                                            <img class="img-fluid d-block d-sm-none" src="/terminalAssets/img/tickets/ticket-face-1-mobile.png?v=0.1" alt="雙人套票">
                                            <p class="ticket-state">
                                                <!--已售完
                                                四人以上未滿八人者<br/>
                                                可依數量加購原價暢行無阻票-->
                                            </p>
                                        </a>
                                    </div>
                                    <div class="type-item type-great-tipsy">
                                        <a href="https://surpriselab.backme.tw/checkout/2534/19985?locale=zh-TW" class="ticket-frame" target="_blank">
                                            <img class="img-fluid d-none d-sm-block" src="/terminalAssets/img/tickets/ticket-face-0.png?v=0.1" alt="雙菜單套票">
                                            <img class="img-fluid d-block d-sm-none" src="/terminalAssets/img/tickets/ticket-face-0-mobile.png?v=0.1" alt="雙菜單套票">
                                            <p class="ticket-state timeLimit">
                                                限量 <span class="highlight total-ticket">500</span> 張 | 剩餘 <span
                                                    class="highlight rest-ticket">XXX</span> 張
                                            </p>
                                        </a>
                                    </div>
                                </div>
                                <p class="ticket-area-note">
                                    以上價格包含 10% 服務費<br/>
                                    現場提供 pairing 加購<br/><br/>
                                    <span class="btn-link" id="js-open-btn">點此查看票券差異比較</span><br/><br/>
                                    單一場次最高可容納人數 12 人，<br/>
                                    若人數超過 12 人可來信客服信箱，由我們協助劃位
                                </p>
                            </div>
                        </div>

                        <!-- 預售劃位流程 -->
                        <div class="section sec-ticket-process">
                            <div class="container">

                                <div class="section-header">
                                    <div class="section-title">
                                        <h3>預售劃位流程</h3>
                                        <h4>Pre-Sale Schedule</h4>
                                    </div>
                                    <p class="section-desc">
                                        無光晚餐採取先預購、後劃位的方式訂位<br/>
                                        依照購買日期順序寄送劃位信
                                    </p>
                                </div>
                                
                                <div class="presale-process">
                                    <div class="step-wrap">
                                        <div class="presale-step">
                                            <div class="step-title">
                                                <div class="step-icon">
                                                    <img class="img-fluid" src="/terminalAssets/img/home/process/process-1.png" alt="先預購">
                                                </div>
                                                <h6 class="color-purple">先預購</h6>
                                            </div>
                                            <div class="step-desc">
                                                <p>
                                                    約好想一起來的那個人<br/>
                                                    購買適合的票券種類
                                                </p>
                                            </div>
                                        </div>
                                        <div class="presale-step">
                                            <div class="step-title">
                                                <div class="step-icon">
                                                    <img class="img-fluid" src="/terminalAssets/img/home/process/process-2.png" alt="後劃位">
                                                </div>
                                                <h6 class="color-purple">後劃位</h6>
                                            </div>
                                            <div class="step-desc">
                                                <p>
                                                    於指定時間收到劃位序號<br/>
                                                    透過線上系統預約用餐時間
                                                </p>
                                            </div>
                                        </div>
                                        <div class="presale-step">
                                            <div class="step-title">
                                                <div class="step-icon">
                                                    <img class="img-fluid" src="/terminalAssets/img/home/process/process-3.png" alt="來體驗">
                                                </div>
                                                <h6 class="color-purple">來體驗</h6>
                                            </div>
                                            <div class="step-desc">
                                                <p>
                                                    於用餐時間前往無光晚餐<br/>
                                                    和夥伴一起進入黑暗
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <!-- 營業時間 -->
                        <div class="section sec-ticket-slots">
                            <div class="container">

                                <div class="section-header">
                                    <div class="section-title">
                                        <h3>營業時間</h3>
                                        <h4>Time Slots</h4>
                                    </div>
                                    <p class="section-desc">
                                        實際可選擇時段請參考票券頁面
                                    </p>
                                </div>

                                <div class="time-session">
                                    <div class="time-session-inner">
                                        <p>平日　17:30｜18:00｜18:30｜19:30｜20:00｜20:30</p>
                                        <hr/>
                                        <p>假日　12:00｜12:30｜13:00</p>
                                        <p>　　　17:30｜18:00｜18:30｜19:30｜20:00｜20:30</p>
                                        <br/>
                                        <p>＊週一週二公休</p>
                                        <div class="feature-btn text-center d-block d-sm-none">
                                            <a class="btn-outline smoothScroll" href="#Ticket-type">我要預約</a>
                                        </div>
                                    </div>
                                </div>

                            </div>
                        </div>

                        <!-- 時程表 -->
                        <div class="section sec-ticket-schedule">
                            <div class="container">

                                <div class="section-header">
                                    <div class="section-title">
                                        <h3>時程表</h3>
                                        <h4>Schedule</h4>
                                    </div>
                                </div>
                                
                                <div class="schedule">
                                    <div class="schedule-table">
                                        <table>
                                            <tr>
                                                <td colspan="2">
                                                    <h6 class="color-purple">劃位時間</h6>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td>08/10 (三) 中午 12:00</td>
                                                <td>前 800 位者</td>
                                            </tr>
                                            <tr>
                                                <td>08/17 (三) 中午 12:00</td>
                                                <td>801-1600 位預約者</td>
                                            </tr>
                                            <tr>
                                                <td>08/24 (三) 中午 12:00</td>
                                                <td>1601-2400 位預約者</td>
                                            </tr>
                                            <tr>
                                                <td>08/31 (三) 中午 12:00</td>
                                                <td>其餘預約者</td>
                                            </tr>
                                            <tr>
                                                <td colspan="2">
                                                    <br class="d-inline d-sm-none"/>
                                                    <p>＊依照購票順序寄送劃位序號，<br class="d-inline d-sm-none"/>收到當下即可劃位</p>
                                                </td>
                                            </tr>
                                        </table>
                                    </div>
                                    <br/><br/>
                                </div>

                                <div class="feature-btn text-center">
                                    <a class="btn-outline smoothScroll" href="#Ticket-type">我要預約</a>
                                </div>
                            </div>
                        </div>
                    </section>

                    <!-- Section3 Rules -->
                    <section class="anchor-section" id="Rules">

                        <h2>Rules</h2>

                        <div class="section sec-rules">
                            <div class="container">

                                <div class="section-header">
                                    <div class="section-title">
                                        <h3>無光規則</h3>
                                        <h4>Rules</h4>
                                    </div>
                                    <p class="section-desc">
                                        若有其他問題，請按下方按鈕
                                    </p>
                                </div>

                                <div class="feature-btn text-center">
                                    <a class="btn-outline" href="rules.html">查看規則</a>
                                </div>

                                <p class="rules-page-contains">
                                    問與答 / 票務規則 / 更改、退費事項
                                </p>
                            </div>
                        </div>
                    </section>

                    <!-- Section4 Contact -->
                    <section class="anchor-section" id="Contact">

                        <h2>Contact</h2>

                        <div class="fullpage section sec-contact">
                            <div class="container">

                                <div class="section-header">
                                    <div class="section-title">
                                        <h3>聯絡我們</h3>
                                        <h4>Contact</h4>
                                    </div>
                                    <p class="section-desc">
                                        有任何問題，可以先至<a href="rules.html">無光規則</a>頁面查看<br/>
                                        若有其他問題，歡迎來信詢問
                                    </p>
                                </div>

                                <div class="contact-sec-wrapper">
                                    <div class="contact-sec">
                                        <div class="contact-sec-title highlight">
                                            聯絡⽅式
                                        </div>
                                        <p>
                                            <a class="default-light" href="mailto:dininginthedark@surpriselab.com.tw">dininginthedark@surpriselab.com.tw</a>
                                        </p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </section>
                </div>
                
                <!-- light -->
                <div class="light light-lg hidden-xs"></div>

                <!-- mask-->
                <div class="mask"></div>

            </div>
        </div>
    </main>

    <!-- Ticket modal -->
    <div id="js-ticket-modal" class="ticket-modal-wrap">
        <div class="ticket-modal-content">
            <div id="js-close-btn" class="close-btn">×</div>

            <div class="ticket-table">
                <img class="img-fluid d-none d-sm-block" src="/terminalAssets/img/other/ticket-compare.png" alt="票券差異比較表">
                <img class="img-fluid d-block d-sm-none" src="/terminalAssets/img/other/ticket-compare-mobile.png" alt="票券差異比較表">
            </div>
        </div>
    </div>

    <!-- Enter modal -->
    <div id="js-enter-modal" class="enter-modal-wrap">
        <div class="enter-modal-content">
            <div id="js-enter-btn" class="enter-btn">Enter</div>

            <!-- board -->
            <div id="js-drag-area" class="drag-wrap">
                <div id="js-arrow-left" class="arrow-wrap style-left">
                    <img src="/terminalAssets/img/home/landing/btn-left.svg" alt="btn-left">
                </div>
                <div id="js-arrow-right" class="arrow-wrap style-right">
                    <img src="/terminalAssets/img/home/landing/btn-right.svg" alt="btn-right">
                </div>
                <div class="logo-wrap">
                    <img src="/terminalAssets/img/general/logo_v.svg" alt="logo">
                </div>
                <div class="board-wrap">
                    <div id="js-rotate-target" class="image-wrap" data-rotate-level="6"></div>
                </div>
            </div>
        </div>
    </div>

    <!-- Social Links -->
    <aside class="social-wrapper">
        <ul>
            <li class="social-link">
                <a href="https://www.facebook.com/surpriselabtw" target="_blank">
                    <i class="icon-fb"></i>
                </a>
            </li>
            <li class="social-link">
                <a href="https://www.instagram.com/surpriselabtw/" target="_blank">
                    <i class="icon-ig"></i>
                </a>
            </li>
        </ul>
    </aside>

    <!-- Bootstrap -->
    <script src="/terminalAssets/js/plugins/jquery-3.3.1.min.js"></script>
    <script src="/terminalAssets/js/plugins/popper.min.js"></script>
    <script src="/terminalAssets/js/plugins/bootstrap-4.1.0/bootstrap.min.js"></script>
    <script src="/terminalAssets/js/plugins/jquery.scrollify.js"></script>

    <!-- Loading Animation JS -->
    <script src="/terminalAssets/js/plugins/imagesloaded/imagesloaded.pkgd.min.js"></script>
    <script>
        document.body.classList.add('render');
        setTimeout(function(){
            imagesLoaded(document.body, function(){
                document.body.classList.remove('loading');
            })
        }, 1000);
    </script>

    <!-- Aos -->
    <script src="/terminalAssets/js/plugins/aos/aos.min.js"></script>

    <!-- Flashlight JS -->
    <script src="/terminalAssets/js/flashlight/imagesLoaded.js"></script>
    <script src="/terminalAssets/js/flashlight/lighting.js"></script>

    <!-- Custom Js -->
    <script src="/terminalAssets/js/index.js?v=0.11"></script>
    <script src="/terminalAssets/js/main.js?v=0.1"></script>
    <script src="/terminalAssets/js/board-modal.js?v=0.11"></script>

</body>
</html>
