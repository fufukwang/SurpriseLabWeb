                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-8">
                                                    <select name="pay_type" id="pay_type" class="form-control" data-edit_type="{{ $order->edit_type or '' }}">
                                                        ...
                                                        <option value="現場購票"@if(isset($order->pay_type) && $order->pay_type=='現場購票') selected @endif>現場購票</option>
...
$('#pay_type').bind('change',function(){
    ...
    if(type == '後台編輯'){
        ...
        etp.html('');
        if(edit_type == 'Line Pay'){ etp.append('<option value="Line Pay" selected>Line Pay</option>'); } else { etp.append('<option value="Line Pay">Line Pay</option>'); }
        if(edit_type == '現金付款'){ etp.append('<option value="現金付款" selected>現金付款</option>'); } else { etp.append('<option value="現金付款">現金付款</option>'); }
        if(edit_type == '現場刷卡'){ etp.append('<option value="現場刷卡" selected>現場刷卡</option>'); } else { etp.append('<option value="現場刷卡">現場刷卡</option>'); }
        if(edit_type == '銀行匯款'){ etp.append('<option value="銀行匯款" selected>銀行匯款</option>'); } else { etp.append('<option value="銀行匯款">銀行匯款</option>'); }
        $('.back_control').show();
    } else if(type == '合作販售'){
        etp.html('');
...
$('select[name=pay_status]').bind('change',function(){
    ...
    if(status == '已付款(部分退款)'){
        ...
        $('.part_control').hide();
        if($('#pay_type').val() == '後台編輯' || $('#pay_type').val() == '現場購票') $('.back_control').show();
    }
});
