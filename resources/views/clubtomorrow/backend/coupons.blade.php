@include('backstage.header',['title' => '明日俱樂部兌換碼列表'])
<!-- =======================
             ===== START PAGE ======
             ======================= -->
        ...
        <div class="wrapper">
            <div class="container">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <h4 class="page-title">明日俱樂部兌換碼列表 </h4>
                    </div>
                </div>
                <!-- Page-Title -->
                            ...
                            <div class="table-rep-plugin">
                                <div class="table-wrapper">
                                    <div class="btn-toolbar">
                                        <div class="btn-group focus-btn-group"><form action="/clubtomorrow/coupons" id="SearchForm">
...
@forelse ($coupons as $row)
                                            <tr id="tr_{{ $row->id }}">
                                                ...
                                                <td>@if($row->o_id > 0) 
                                                    {{ App\model\club\order::where('sn',$row->o_id)->first()->created_at }}
                                                @else 尚未兌換
                                                @endif</td>
...
$(function(){
    ...
    $('.btn-danger').bind('click',function(){
        ...
        if(confirm(txt + "確定要刪除此優惠券")) {
             $.ajax({
                url: '/clubtomorrow/coupon/'+id+'/delete',
                method: 'delete',
                dataType:'json'
            }).done(function(data){
                swal(data.message);
