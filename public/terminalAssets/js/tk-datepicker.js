...
var allData, allowDates;
var dateSite = {};
var loading = true;
var displayedMonths = [];
var openMonth = {};

function init(startYear, startMonth) {
    ...
    document.querySelectorAll('.tk-datepicker-body .grid').forEach(function(el) {
        ...
        if (allowDates.indexOf(date) < 0) {
            ...
            if(dateSite[date]<50){
                el.classList.add('sold-out-soon');
                el.title = '即將完售';
            } else if(dateSite[date]>=50){
                el.classList.add('still-vacancy');
                el.title = '好評熱賣';
            }
...
$.get('/terminal/GetAjaxData',{
    ...
    allData = data;
    allowDates = allData.map(function(item){
        return item.day;
    });
    ...
    $('.tk-datepicker').css({height: 'auto', opacity: 1});
