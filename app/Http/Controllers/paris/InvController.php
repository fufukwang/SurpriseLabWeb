...
class InvController extends WebController
{
    ...
    private function inv_sent($post_data_array){
        ...
        $key = env('PARIS_KEY'); //商店專屬串接金鑰
        //HashKey 值
        $iv = env('PARIS_IV'); //商店專屬串接金鑰 HashIV 值
        if (phpversion() > 7) {
        ...
        $MerchantID = env('PARIS_INV_ID'); //商店代號
    ...
    public function muInvOpen(Request $request){
        try{
            ...
            return redirect('/paris/print?')->with('message','發票開立完成!');
        } catch (Exception $exception) {
            Log::error($exception);
            return redirect('/paris/print?')->with('message','發票開立失敗!');
        }
    }
    // 單一發票開立
