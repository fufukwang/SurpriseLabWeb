        ...
        try {
            if($request->has('id') && $request->has('sn')){
                ...
                if($order){
                    ...
                    return "<script>alert('活動不在範圍!');window.location='/paris/index.html';</script>";
                }
            } else {
                return "<script>alert('沒編號!');window.location='/paris/index.html';</script>";
            }
        } catch (Exception $exception) {
            Log::error($exception);
            return "<script>alert('有錯誤!');window.location='/paris/index.html';</script>";
        }
    }
    // 存入 team 的資料並寄送信件
    ...
    public function getNameFix(Request $request){
        try{
            if($request->has('d') && $request->has('e')){
                ...
                if($tm){
                    $order = order::leftJoin('paris_pro', 'paris_pro.id', '=', 'paris_order.pro_id');
                    $order = $order->select('name','day','rang_start','paris_order.id','sn')->whereIn('pay_status',['已付款','已付款(部分退款)'])
                        ->whereRaw("MD5(paris_order.id)='".$order_id."' AND UNIX_TIMESTAMP(CONCAT(day,' ',rang_start))>=UNIX_TIMESTAMP()")
                        ->first();
                    if($order){

                        return view('paris.frontend.nameFix',compact('order','tm'));
                    } else {
                        return "<script>alert('沒有需要修改的資料!');window.location='/paris/index.html';</script>";
                    }
                } else {
                    return "<script>alert('沒有需要修改的資料!');window.location='/paris/index.html';</script>";
                }
            } else {
                return "<script>alert('參數錯誤!');window.location='/paris/index.html';</script>";
            }
        } catch (Exception $exception) {
            Log::error($exception);
            return "<script>alert('名冊中已有您的姓名!');window.location='/paris/index.html';</script>";
        }
    }
    ...
    public function postReSendMail(Request $request){
        try{
            ...
            if($toData['template'] == 'D7'){
                $ord = order::leftJoin('paris_pro', 'paris_pro.id', '=', 'paris_order.pro_id')->select('pople','day','rang_start','vegetarian','meat_eat','no_beef','no_pork','no_nut_m','no_shell','no_nut_v','no_fish','need_english')->find($request->id);
