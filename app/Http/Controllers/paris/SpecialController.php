...
class SpecialController extends WebController
{
    ...
    public function getHome(Request $request){
        ...
        $data = [
            'count' => $count,
            't6'    => json_decode(setting::where('slug','paris_sp_t6')->first()->json,true),
            'money' => json_decode(setting::where('slug','paris_sp_money')->first()->json,true),
        ];

        return view('paris.frontend.booking_special',compact('data'));
    }
    // 訂單處理送往金流 ( 藍新 )
    public function postOrderByNeweb(Request $request){
        try {
            ...
            if($people>$act->Count){
                Log::error('人數滿了');
                return view('paris.frontend.booking_fail',['sp'=>1]);
            }
            ...
            */
            ...
            if($request->has('discount')){
                ...
                $discount_list = json_decode(setting::where('slug','paris_sp_discount')->first()->json,true);
    ...
    public function getSpecialSetting(Request $request){
        try{
            ...
            $data = [
                't6'    => json_decode(setting::where('slug','paris_sp_t6')->first()->json,true),
                'money' => json_decode(setting::where('slug','paris_sp_money')->first()->json,true),
                'setting' => json_decode(setting::where('slug','paris_setting')->first()->json,true),
            ];


            return view('paris.backend.settings.special',compact('data'));
        } catch (Exception $exception) {
            Log::error($exception);
            abort(404);
        }
    ...
    public function getDiscountSetting(Request $request){
        try{
            ...
            $data = [
                'pay' => json_decode(setting::where('slug','paris_pay_discount')->first()->json,true),
                'sp'  => json_decode(setting::where('slug','paris_sp_discount')->first()->json,true),
            ];


            return view('paris.backend.settings.discount',compact('data'));
    ...
    public function postSettingStore(Request $request){
        ...
        try{
            ...
            if($request->has('slug')){
                if($request->slug == 'paris_sp_t6'){
                    $is_store = setting::where('slug','paris_sp_t6')->update(['json'=>json_encode(['number'=> $request->number])]);
                    if($is_store) $return['success'] = true;
                } elseif($request->slug == 'paris_sp_money'){
                    $is_store = setting::where('slug','paris_sp_money')->update(['json'=>json_encode([
                        [ 'number' => 1,'money' => $request->t1_money ],
                        [ 'number' => 2,'money' => $request->t2_money ],
                        [ 'number' => 6,'money' => $request->t6_money ],
                    ])]);
                    if($is_store) $return['success'] = true;
                } elseif($request->slug == 'paris_setting'){
                    $is_store = setting::where('slug','paris_setting')->update(['json'=>json_encode([
                        "max" => $request->max,
                        "pay_max_date" => $request->pay_max_date
                    ])]);
                    if($is_store) $return['success'] = true;
                } elseif($request->slug == 'paris_sp_discount'){
                    $is_store = setting::where('slug','paris_sp_discount')->update(['json'=>$request->obj]);
                    if($is_store) $return['success'] = true;
                } elseif($request->slug == 'paris_pay_discount'){
                    $is_store = setting::where('slug','paris_pay_discount')->update(['json'=>$request->obj]);
                    if($is_store) $return['success'] = true;
                }
            }
