...
class OrderController extends WebController
{
    ...
    public function Orders(Request $request,$id){
        ...
        $order = $order->get();
        return view('paris.backend.orders',compact('order'));
    }
    ...
    public function OrderUpdate(Request $request,$id){
        ...
        if($request->has('qxx') && $request->qxx != ''){
            return redirect('/paris/print?'.$request->qxx)->with('message','編輯完成!');
        } else {
            return redirect('/paris/orders/'.$order->pro_id)->with('message','編輯完成!');
        }
    }
    ...
    public function Appointment(Request $request,$pro_id){
        try {
            $pro = pro::find($pro_id);
            return view('paris.backend.orderAppointment',compact('pro_id','pro'));
        } catch (Exception $exception) {
            Log::error($exception);
            return redirect('/paris/pros?')->with('message','此編號無座位表!');
        }
    }
            ...
            */
            ...
            if($act->special) {
                ...
                if($people!=1 && $people!=2 && $people!=6){
                    return redirect('/paris/pros?')->with('message','新增失敗!特別場次請選擇 1、2、6符合票券人數');
                }
                $sp_money = json_decode(setting::where('slug','paris_sp_money')->first()->json,true);
                if($people == 1){
            ...
            Log::error($exception);
            return redirect('/paris/pros?')->with('message','新增失敗!');
        }
    }
    ...
    public function orderImportXls(Request $request){
        ...
        try{
            ...
            if ($validator->fails()) {
                return redirect('/paris/backmes')->with('message','新增失敗!(請檢查檔案)');
            } else {
            ...
            return redirect('/paris/print')->with('message',"新增完成!!共新增{$count}筆資料<br>{$message}");
        } catch (Exception $exception) {
            Log::error($exception);
            return redirect('/paris/print')->with('message','新增失敗!');
        }
    }
    ...
    public function Print(Request $request){
        ...
        return view('paris.backend.print',compact('order','request'));
    }
    ...
    public function Table(Request $request){
        ...
        return view('paris.backend.table',compact('order','request'));
    }
    ...
    public function XlsEmailDataOuput(Request $request){
        ...
        $order->whereRaw("(paris_order.pay_status='已付款' OR paris_order.pay_status='已付款(部分退款)' OR (paris_order.pay_type='現場付款' AND paris_order.pay_status<>'取消訂位'))");
        $order = $order->orderBy('paris_order.updated_at','desc')->get();
        $cellData = $order->toArray();
    ...
    private function orderQuery(Request $request,$isTable=false){
        ...
        if($request->has('pay_status') && $request->pay_status=='已預約'){
            ...
            $order->where('paris_order.pay_status',$request->pay_status);  
        } 
        ...
        if($request->has('no_inv') && $request->no_inv == 1){
            // $order->whereRaw("(SELECT COUNT(paris_inv.id) FROM paris_inv WHERE is_cancal=0 AND paris_order.id=paris_inv.order_id)=0 AND pay_status IN ('已付款','已付款(部分退款)')");
            $order->whereIn('paris_order.pay_status',['已付款','已付款(部分退款)']);  
            $order->leftJoin('paris_inv',function($join){
                $join->on('paris_order.id', '=', 'paris_inv.order_id');
        ...
        } else { $order = $order->orderBy('paris_order.updated_at','desc'); }
        return $order;
    }
