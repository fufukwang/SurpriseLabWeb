...
class BackController extends Controller
{
    ...
    public function BackMes(Request $request){
        ...
        if($request->isMethod('post') && $request->has('id')){
            ...
            return redirect('/terminal/backmes'.$par)->with('message','已寄送完成!');
        }
...
// (SELECT COUNT(id) FROM(tgt2coupon) WHERE code LIKE '%{$$request->search}%' AND tgt2coupon.b_id=tgt2backme.id)
        ...
        //dd(DB::getQueryLog());
        return view('terminal.backend.BackMes',compact('mes','request','quart'));
    }
    public function NotUseXls(Request $request){
        $backmes = backme::select('name','email','tel','p2','p4','detail','manage')->whereRaw("(SELECT COUNT(id) FROM(terminalcoupon) WHERE o_id=0 AND terminalcoupon.b_id=terminalbackme.id)>0")->get()->toArray();
    ...
    public function ProEdit(Request $request,$id){
        ...
        return view('terminal.backend.pro',compact('pro'));
    }
    public function ProUpdate(Request $request,$id){
        ...
        return redirect('/terminal/pros')->with('message','編輯完成!');
    }
    ...
    public function Gift(Request $request){
        try{
            ...
            Log::error($exception);
            return redirect('/terminal/backmes')->with('message','新增失敗!');
        }
    }
...
//dd($request->file('xlsx')->getMimeType());
        try{
            ...
            if ($validator->fails()) {
                return redirect('/terminal/backmes')->with('message','新增失敗!(請輸入季度或檢查檔案)');
            } else {
            ...
            return redirect('/terminal/backmes')->with('message','新增完成!!共新增'.$i.'筆資料');
        } catch (Exception $exception) {
            Log::error($exception);
            return redirect('/terminal/backmes')->with('message','新增失敗!');
        }
