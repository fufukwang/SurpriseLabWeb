    ...
    private function getOrderSearch(Request $request,$isTable=false){
        try {
            ...
            if($request->has('no_inv') && $request->no_inv == 1){
                $order->whereRaw("(SELECT COUNT(terminalinv.id) FROM terminalinv WHERE is_cancal=0 AND terminalorder.id=terminalinv.order_id)=0 AND pay_status IN ('已付款','已付款(部分退款)') AND pay_type IN ('信用卡','後台編輯','現場購票')");
            }
