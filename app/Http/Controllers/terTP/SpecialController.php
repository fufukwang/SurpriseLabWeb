...
class SpecialController extends WebController
{
    ...
    public function postOrderByNeweb(Request $request){
        try {
            ...
            if($people>$act->Count){
                Log::error('人數滿了');
                return view('terTP.frontend.booking_fail',['sp'=>1]);
            }
            ...
            */
            ...
            $sentSuccess = false;
            if($data['money'] == 0){
                return view('terTP.frontend.booking_success');
            } else {
                $comments = "無光晚餐";
                if($cut2>0){
    ...
    public function getSpecialSetting(Request $request){
        try{
            ...
            return view('terTP.backend.settings.special',compact('data'));
        } catch (Exception $exception) {
            Log::error($exception);
            abort(404);
        }
    ...
    public function getDiscountSetting(Request $request){
        try{
            ...
            return view('terTP.backend.settings.discount',compact('data'));
