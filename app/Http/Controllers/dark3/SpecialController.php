...
class SpecialController extends Controller
{
        ...
        try {
            ...
            */
            ...
            if($request->has('discount')){
                ...
                $discount_list = json_decode(setting::where('slug','dark3_sp_discount')->first()->json,true);
    ...
    public function getDiscountSetting(Request $request){
        try{
            ...
            $data = [
                'pay' => json_decode(setting::where('slug','dark3_pay_discount')->first()->json,true),
                'sp'  => json_decode(setting::where('slug','dark3_sp_discount')->first()->json,true),
            ];
    ...
    public function postSettingStore(Request $request){
        ...
        try{
            ...
            if($request->has('slug')){
                if($request->slug == 'dark3_sp_t6'){
                    $is_store = setting::where('slug','dark3_sp_t6')->update(['json'=>json_encode(['number'=> $request->number])]);
                    if($is_store) $return['success'] = true;
                } elseif($request->slug == 'dark3_sp_money'){
                    $is_store = setting::where('slug','dark3_sp_money')->update(['json'=>json_encode([
                    ...
                    $is_store = setting::where('slug','dark3_sp_discount')->update(['json'=>$request->obj]);
                    if($is_store) $return['success'] = true;
                } elseif($request->slug == 'dark3_pay_discount'){
                    $is_store = setting::where('slug','dark3_pay_discount')->update(['json'=>$request->obj]);
                    if($is_store) $return['success'] = true;
                }
            }
